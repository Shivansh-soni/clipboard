...
export default function RenderItems(props: RenderItemsProps) {
  ...
  const handleImageClick = (item: ClipboardItem) => {
    if (
      item.type === "image" ||
      (item.type === "file" && item.fileType?.startsWith("image/"))
    ) {
      setPreviewImage({
        url: `/api/files/${item.content}`,
                  ...
                  <div className="flex items-center gap-3 p-3 bg-gray-800 rounded-lg">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-xs text-gray-400">
                        {formatFileSize(item.fileSize || 0)} • {item.fileType}
                      </p>
                    </div>
                    <Button
                      variant="ghost"
                      ...
                      onClick={(e) => {
                        ...
                        if (item.fileType?.startsWith("image/")) {
                          ...
                          window.open(`/api/files/${item.content}`, "_blank");
                        }
                      }}
                      title={
